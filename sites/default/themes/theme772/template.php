...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme772
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme772_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme772_html_head_alter(&$head_elements) {
 ...
 */
function theme772_preprocess_search_block_form(&$vars) {
 ...
 */
function theme772_preprocess(&$vars, $hook) {
  $vars['theme772_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme772_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme772_doctype();
  $vars['rdf'] = _theme772_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme772_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme772_id_safe('section-' . $section);
 ...
 */
function theme772_preprocess_page(&$vars) {
 ...
 */
function theme772_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme772') . '/css/maintenance-page.css');
}
 ...
 */
function theme772_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme772_preprocess_block(&$vars, $hook) {
 ...
 */
function theme772_menu_tree($vars) {
 ...
 */
function theme772_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme772_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme772_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme772_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme772_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme772_breadcrumb_separator');
 ...
 * @return bool
 */
function theme772_tabs_float() {
  $float = (bool) theme_get_setting('theme772_tabs_float');
  $float_node = (bool) theme_get_setting('theme772_tabs_node');
...
function theme772_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme772_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme772_id_safe($element['#title']);
 ...
 */
function theme772_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme772_menu_local_tasks(&$vars) {
  $output = '';
