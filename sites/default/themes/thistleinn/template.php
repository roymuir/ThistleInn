...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for thistleinn
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('thistleinn_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function thistleinn_html_head_alter(&$head_elements) {
 ...
 */
function thistleinn_preprocess_search_block_form(&$vars) {
 ...
 */
function thistleinn_preprocess(&$vars, $hook) {
  $vars['thistleinn_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function thistleinn_preprocess_html(&$vars) {
  
  $vars['doctype'] = _thistleinn_doctype();
  $vars['rdf'] = _thistleinn_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = thistleinn_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = thistleinn_id_safe('section-' . $section);
 ...
 */
function thistleinn_preprocess_page(&$vars) {
 ...
 */
function thistleinn_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'thistleinn') . '/css/maintenance-page.css');
}
 ...
 */
function thistleinn_preprocess_node(&$vars) {
  // Striping class
  ...
  $vars['submitted'] = t('Submitted by !username on ', array('!username' => $vars['name']));
	$vars['submitted_date'] = format_date($vars['created'], 'custom', 'd F, Y');
  $vars['submitted_pubdate'] = format_date($vars['created'], 'custom', 'd F, Y');
 ...
 */
function thistleinn_preprocess_block(&$vars, $hook) {
 ...
 */
function thistleinn_menu_tree($vars) {
 ...
 */
function thistleinn_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function thistleinn_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('thistleinn_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('thistleinn_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('thistleinn_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('thistleinn_breadcrumb_separator');
 ...
 * @return bool
 */
function thistleinn_tabs_float() {
  $float = (bool) theme_get_setting('thistleinn_tabs_float');
  $float_node = (bool) theme_get_setting('thistleinn_tabs_node');
...
function thistleinn_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function thistleinn_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = thistleinn_id_safe($element['#title']);
 ...
 */
function thistleinn_preprocess_menu_local_task(&$vars) {
 ...
 */
function thistleinn_menu_local_tasks(&$vars) {
  $output = '';
